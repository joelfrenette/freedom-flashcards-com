...
export default function ImCuriousPage() {
  const [flashcards, setFlashcards] = useState(allImCuriousFlashcards)
  const [currentPage, setCurrentPage] = useState(1)
  const cardsPerPage = 5
  const totalPages = Math.ceil(flashcards.length / cardsPerPage)

  useEffect(() => {
    const savedData = localStorage.getItem("flashcards-im-curious")
    if (savedData) {
      try {
        ...
        setFlashcards(allImCuriousFlashcards)
      }
    }
  }, [])
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      <main className="flex-1 container mx-auto px-4 py-12">
        <div className="mb-8">
          <h1 className="text-4xl font-bold mb-2">I'm Just Curious</h1>
          <p className="text-xl text-gray-600 mb-4">Provocative questions to spark critical thinking</p>
          <p className="text-gray-600">
            Thought-provoking questions that challenge assumptions and encourage deeper examination of popular
            narratives.
          </p>
        </div>
