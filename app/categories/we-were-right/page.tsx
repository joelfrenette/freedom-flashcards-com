...
export default function WeWereRightPage() {
  const [flashcards, setFlashcards] = useState(allWeWereRightFlashcards)
  const [currentPage, setCurrentPage] = useState(1)
  const cardsPerPage = 5
  const totalPages = Math.ceil(flashcards.length / cardsPerPage)

  useEffect(() => {
    const savedData = localStorage.getItem("flashcards-we-were-right")
    if (savedData) {
      try {
        ...
        setFlashcards(allWeWereRightFlashcards)
      }
    }
  }, [])
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      <main className="flex-1 container mx-auto px-4 py-12">
        <div className="mb-8">
          <h1 className="text-4xl font-bold mb-2">We Were Right About</h1>
          <p className="text-xl text-gray-600 mb-4">Vindication of early skeptics and truth-tellers</p>
          <p className="text-gray-600">
            Explore the issues where early skeptics and independent thinkers were proven correct despite being dismissed
            or censored.
          </p>
        </div>
