...
export default function Freedom101Page() {
  const [flashcards, setFlashcards] = useState(allFreedom101Flashcards)
  const [currentPage, setCurrentPage] = useState(1)
  const cardsPerPage = 5
  const totalPages = Math.ceil(flashcards.length / cardsPerPage)

  useEffect(() => {
    const savedData = localStorage.getItem("flashcards-freedom-101")
    if (savedData) {
      try {
        ...
        setFlashcards(allFreedom101Flashcards)
      }
    }
  }, [])
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      <main className="flex-1 container mx-auto px-4 py-12">
        <div className="mb-8">
          <h1 className="text-4xl font-bold mb-2">Freedom 101</h1>
          <p className="text-xl text-gray-600 mb-4">Intro deck for newcomers to conservative thought</p>
          <p className="text-gray-600">
            Learn about America's founding documents and principles explained in simple terms. Discover the wisdom of
            the Federalist Papers, Bill of Rights, Constitution, and landmark Supreme Court cases.
          </p>
        </div>
