...
export default function WhyDoYouFeelPage() {
  const [flashcards, setFlashcards] = useState(allWhyDoYouFeelFlashcards)
  const [currentPage, setCurrentPage] = useState(1)
  const cardsPerPage = 5

  useEffect(() => {
    const savedData = localStorage.getItem("flashcards-why-do-you-feel")
    if (savedData) {
      try {
        ...
        setFlashcards(allWhyDoYouFeelFlashcards)
      }
    }
  }, [])
  ...
  const displayStartIndex = indexOfFirstCard + 1
  const displayEndIndex = Math.min(indexOfLastCard, flashcards.length)
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      <main className="flex-1 container mx-auto px-4 py-12">
        <div className="mb-8">
          <h1 className="text-4xl font-bold mb-2">Why Do You Feel</h1>
          <p className="text-xl text-gray-600 mb-4">Questions that challenge emotional reasoning</p>
          <p className="text-gray-600">
            Explore questions that encourage people to examine the sources and validity of their emotional reactions to
        ...
        <div className="mb-6 text-sm text-gray-600">
          Showing {displayStartIndex}-{displayEndIndex} of {flashcards.length} flashcards (Page {currentPage} of{" "}
          {totalPages})
        </div>
        <div className="space-y-6">
          {currentFlashcards.map((flashcard) => (
            <QuestionFlashcard key={flashcard.id} flashcard={flashcard} />
          ))}
        </div>
