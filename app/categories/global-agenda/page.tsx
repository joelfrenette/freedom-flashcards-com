...
export default function GlobalAgendaPage() {
  const [flashcards, setFlashcards] = useState(allGlobalAgendaFlashcards)
  const [currentPage, setCurrentPage] = useState(1)
  const cardsPerPage = 5
  const totalPages = Math.ceil(flashcards.length / cardsPerPage)

  useEffect(() => {
    const savedData = localStorage.getItem("flashcards-global-agenda")
    if (savedData) {
      try {
        ...
        setFlashcards(allGlobalAgendaFlashcards)
      }
    }
  }, [])
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      <main className="flex-1 container mx-auto px-4 py-12">
        <div className="mb-8">
          <h1 className="text-4xl font-bold mb-2">Global Agenda Exposed</h1>
          <p className="text-xl text-gray-600 mb-4">Understanding worldwide coordinated narratives</p>
          <p className="text-gray-600">
            Examine international organizations, global initiatives, and coordinated efforts that shape policy and
