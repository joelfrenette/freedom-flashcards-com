...
export default function DoubleStandardsPage() {
  const [flashcards, setFlashcards] = useState(allDoubleStandardsFlashcards)
  const [currentPage, setCurrentPage] = useState(1)
  const cardsPerPage = 5
  const totalPages = Math.ceil(flashcards.length / cardsPerPage)

  useEffect(() => {
    const savedData = localStorage.getItem("flashcards-double-standards")
    if (savedData) {
      try {
        ...
        setFlashcards(allDoubleStandardsFlashcards)
      }
    }
  }, [])
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      <main className="flex-1 container mx-auto px-4 py-12">
        <div className="mb-8">
          <h1 className="text-4xl font-bold mb-2">Double Standards Exposed</h1>
          <p className="text-xl text-gray-600 mb-4">Highlighting hypocrisy in rules and narratives</p>
          <p className="text-gray-600">
            Examine the glaring double standards in media coverage, political treatment, and social expectations.
          </p>
        </div>
